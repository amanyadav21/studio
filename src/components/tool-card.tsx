...
export function ToolCard({ tool, isFavorite, onToggleFavorite, isInBundle, onToggleBundle }: ToolCardProps) {
  ...
  return (
    <Link href={`/tool/${tool.id}`} className="group block h-full">
      <Card className="flex h-full flex-col overflow-hidden rounded-lg transition-all duration-300 group-hover:shadow-2xl group-hover:-translate-y-1.5">
          ...
          <div className="absolute top-2 right-2 flex flex-col gap-2 opacity-0 transition-opacity duration-300 group-hover:opacity-100">
            <TooltipProvider>
              <Tooltip>
                ...
                <TooltipContent>{isInBundle ? "Remove from bundle" : "Add to bundle"}</TooltipContent>
              </Tooltip>
            </TooltipProvider>
            <TooltipProvider>
              <Tooltip>
        ...
        <CardHeader className="flex-grow p-4">
          <Badge variant="outline" className="mb-2 w-fit">{tool.category}</Badge>
          <CardTitle className="text-lg font-semibold">{tool.name}</CardTitle>
          <CardDescription className="mt-1 line-clamp-2 text-sm">{tool.description}</CardDescription>
        </CardHeader>
        <CardFooter className="p-4 pt-0">
           <div className={cn(buttonVariants({ variant: 'default' }), "w-full font-semibold")}>
            Launch Now
